...
roof = gpd.read_file(data.joinpath(r'roof.geojson'))
...
def calculate_z(id, left, top, triangles, meta):
    ...
    dest = data.joinpath("dom/{}.tif".format(id))
    try:
...
if __name__ == '__main__':
    ...
    for index, row in grid.iterrows():
        ...
        if data.joinpath('dom/{}.tif'.format(id)).exists():
            continue
        triangles = roof[roof.intersects(geom.buffer(1))].copy()
